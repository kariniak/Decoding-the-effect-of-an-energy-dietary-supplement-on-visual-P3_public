...
for isub = 1:length(subjects)
   ...
   filename = fullfile(analyzed_eeg_dir, subj_str, sprintf('%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_rename_merge_elist_bins.set', subj_str)); 
   ...
   % Create ERP epochs
   EEG = pop_loadset('filename', sprintf('%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_rename_merge_elist_bins.set', subj_str), 'filepath', subj_anal_dir);
   EEG = pop_epochbin( EEG , [-200.0  800.0],  'pre'); % with baseline correction
   EEG = eeg_checkset( EEG );
   eeglab redraw;
   %end

   %Save dataset:
   EEG = pop_saveset(EEG, 'filename',sprintf('%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_rename_merge_elist_bins_%s.set', subj_str, set_str_out),'filepath',subj_anal_dir);
