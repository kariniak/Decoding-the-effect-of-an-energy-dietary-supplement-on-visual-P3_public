...
for isub = 1:length(subjects)
   ...
   % Decode
   MVPC = pop_decoding( BEST , 'Channels', [ 1:30], 'classcoding', 'OneVsAll', 'Classes', [ 2:2:6], 'Decode_Every_Npoint', [ 2], 'DecodeTimes', [ -195.312 796.875], 'EqualizeTrials', 'floor', 'FloorValue', [ 13], 'Method', 'SVM', 'nCrossblocks', [ 3], 'nIter', [ 100], 'ParCompute', 'on' );
   eeglab redraw;
   
   % Save decoded file
   MVPC = pop_savemymvpc(MVPC, 'mvpcname', sprintf('%s_MVPC_f3_t13_excludeepochs', subj_str), 'filename', sprintf('%s_MVPC_f3_t13_excludeepochs.mvpc', subj_str), 'filepath', subj_anal_dir, 'Warning', 'on');
