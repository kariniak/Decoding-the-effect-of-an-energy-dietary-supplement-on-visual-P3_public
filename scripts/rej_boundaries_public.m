...
for isub = 1:length(subjects)
   ...
   filename = fullfile(analyzed_eeg_dir, subj_str, sprintf('%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_rename_merge_elist_bins_epoch.set', subj_str)); 
   if ~exist(filename, 'file')
      fprintf('%s does not exist - skipping\n', filename) 
      continue;
   end
        
   % Reject epochs with "boundary" event
   EEG = pop_loadset('filename', sprintf('%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_rename_merge_elist_bins_epoch.set', subj_str), 'filepath', subj_anal_dir);
   EEG = pop_selectevent( EEG, 'type',-99,'deleteevents','off','deleteepochs','on','invertepochs','on');
   EEG = eeg_checkset( EEG );
   eeglab redraw;
   %end

   %Save dataset:
   EEG = pop_saveset(EEG, 'filename',sprintf('%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_rename_merge_elist_bins_epoch_%s.set', subj_str, set_str_out),'filepath',subj_anal_dir);
