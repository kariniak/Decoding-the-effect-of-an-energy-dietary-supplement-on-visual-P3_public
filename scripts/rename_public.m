...
for isub = 1:length(subjects)
    ...
    for iset = 1:num_runs % loop across runs
        filename = fullfile(analyzed_eeg_dir, subj_str, sprintf('%s_run%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet.set', subj_str, num2str(iset))); 
        ...
        EEG = pop_loadset('filename', sprintf('%s_run%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet.set', subj_str, num2str(iset)), 'filepath', subj_anal_dir);
    ...
    %Save dataset:
    EEG = pop_saveset( EEG, 'filename',sprintf('%s_run%s_elecloc_hpflt01Hz_crazy_elist_bins_rerefMeanMast_ica_rejICs_blinkdet_%s.set', subj_str, num2str(iset), set_str_out),'filepath',subj_anal_dir);
    EEG = eeg_checkset( EEG );
